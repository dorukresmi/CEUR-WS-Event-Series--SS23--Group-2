...
from eventseries.dblp.DblpParsing import parse_venue_div
...
class DblpScraper:
    ...
    def crawl_all_events_without_series(self,
                                        path_to_events: Path = resources.files(
                                            'eventseries.resources') / 'EventsWithoutSeries.json'):
    ...
    def crawl_conf_index(self, index_url: str, pos: int = 0, driver_instance: Optional[WebDriver] = None):
        ...
        a_elements, opt_next_link = DblpScraper.scrape_conf_index(conf_index_url=full_url, driver_instance=driver)
