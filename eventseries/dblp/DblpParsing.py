...
from eventseries.dblp.EventClasses import Event, DblpEvent, EventSeries
from eventseries.dblp.VenueInformationClasses import VenueInformation, IsPartOf, NameWithOptionalReference, \
    Predecessor, Related, Status, Successor, YearRange
from eventseries.dblp.VenueInformationClasses import HasPart
from eventseries.dblp.dblp import DblpContext
...
class EventSeriesParser:
    ...
    def parse_event_tag(event_h2: Tag) -> List[Event]:
        ...
        return [
            Event(title=event_title, year=opt_year, location=opt_location,
...
def event_series_from_soup(soup: BeautifulSoup, given_dblp_id: Optional[str] = None):
    ...
    # Extract dblp_id
    dblp_id = given_dblp_id if given_dblp_id is not None else \
    ...
    if opt_abbreviation is None and re.search(r'\((\w+)\)', name):
        print("Possible abbreviation longer than 20 characters: " + re.search(r'\((\w+)\)', name).groups()[0])
    abbreviation = None
    ...
    if abbreviation:
        ...
        if non_word is not None:
            print(f"Suspicious characters found in abbreviation: '{non_word.group()}'"
    ...
    events: List[Event] = list(
        itertools.chain.from_iterable([EventSeriesParser.parse_event_tag(event_h2) for event_h2 in event_h2_list]))
...
def load_event_series(path: Path = resources.files('eventseries.resources') / 'dblp_event_series.pickle') -> List[
    EventSeries]:
...
if __name__ == '__main__':
    store_event_series(DblpContext())
