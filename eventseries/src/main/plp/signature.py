...
from plp.parsing import Category
...
class EnumCategory(Category):
    ...
    def getYamlPath(self, lookupName: str):
        ...
        filepath = (parent / f"resources/{lookupName}.yaml")
        path = str(filepath)
        return path
...
class RomanNumeral(pp.OneOrMore):
    ...
    def __init__(self, limit: int = None, name: str = None):
        ...
        expressions = [self.romanNumeralLiteral(numeralString, value) for numeralString, value in map if
...
class VolumeCategory(Category):
    ...
    VOLUME_VALUE_ALPHA = pp.Char(pp.alphas).setParseAction(pp.tokenMap(lambda value: ord(value.lower()) - 96)).setName(
        "alphanumeric")
    VOLUME_VALUE = pp.Group(VOLUME_VALUE_NUMERIC | RomanNumeral(limit=100, name="roman reduced") | VOLUME_VALUE_ALPHA)(
        "value")
