...
import pandas as pd
from eventseries.dblp.DblpParsing import load_event_series

def get_sameTime_cfpEvents(df_wikidata,df_cfp):
    ll = []
    for index, row in df_wikidata.iterrows():
        df1 = df_cfp[df_cfp['startDate'] == df_wikidata.loc[index, 'startTime']]['eventId']
...
def get_similary_cfpEvents_Title(df_wikidata):
    ...
    for index, row in df_wikidata[
        df_wikidata.wikiCfpId.notna() & df_wikidata.same_time_cfpTitle.notna()].iterrows():
        id_list = []
        t_list = []
        count = 0
...
def get_similary_cfpEvents_Acronym(df_wikidata):
    ...
    for index, row in df_wikidata[df_wikidata.same_time_cfpId.notna()].iterrows():
        ...
        count = 0
        for item in row['same_time_cfpId']:
            p_matched_acronym = df_wikidata[df_wikidata['eventId'] == item]['acronym'].values[0]
...
def cfp_dblp_Link():
    ...
    for index, row in df_dblp.iterrows():
        l_id = df_series_wikicfp[df_series_wikicfp['dblpSeriesId'] == row['dblp_id']]['wikiCfpId'].values
        l_match = []

        if len(l_id) > 1:
            for id in l_id:
                title_cfp = df_series_wikicfp[df_series_wikicfp['wikiCfpId'] == id]['title'].values
    ...
    for index, row in df_dblp[df_dblp.seriesIds_cfp.notna()].iterrows():
        ...
        for id in l_id:
            ...
            for ind, ro in df_match.iterrows():
                l_event_cfp.append({"CFPeventID": ro['eventId'], "CFPtitle": ro['title'], "year": ro['year']})
