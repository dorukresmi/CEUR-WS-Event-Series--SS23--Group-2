...
class Events(object):
    def query(self):
        ...
        file = (resources.files('eventseries.resources') / "events.json").open("w", encoding="utf-8")
    ...
    def event_series_query(self):
        ...
        file = (resources.files('eventseries.resources') / "event_series.json").open("w", encoding="utf-8")
        file.write(response.text)
        file.close()
